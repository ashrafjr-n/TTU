...
@section('title', __('auth_forms.forgot_password.heading'))
...
<div class="min-h-screen flex flex-col items-center justify-center px-4 py-14 bg-ttu-cream">
    ...
    <div class="w-full max-w-md rounded-[2.5rem] neu-raised-white p-8">
        ...
        <div class="text-center mb-6">
            <h2 class="font-display text-2xl font-extrabold">{{ __('auth_forms.forgot_password.heading') }}</h2>
        </div>

        <p class="text-sm text-ttu-gray leading-relaxed mb-6">
            {{ __('auth_forms.forgot_password.intro') }}
        </p>
        ...
        <form method="POST" action="{{ route('password.email') }}" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-ttu-black mb-1.5">{{ __('auth_forms.forgot_password.email') }}</label>
            ...
            <button type="submit" class="w-full btn-hero justify-center">
                {{ __('auth_forms.forgot_password.submit') }}
            </button>
        </form>
